...
class TicketmasterClientSpec extends PlaySpec with Results with PrivateMethodTester with GuiceOneAppPerSuite {

  private val settings                      = Settings.apply
  implicit val ec: ExecutionContextExecutor = global

  def withRealClient(testCode: TicketmasterClient => Any): Any = {
    ...
    try {
      testCode(TicketmasterClient(ws, settings)(ec)) // loan the client
    } finally ws.close()
